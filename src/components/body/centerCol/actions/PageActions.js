import { FETCH_POKEMONS_REQUEST, FETCH_POKEMONS_SUCCESS, FILTER_POKEMONS } from '../constants/Page';
...
function receivePokemons(json) {
    const pokemons = json.results.map(pokemon => {
        let { url } = pokemon;
        pokemon.id = url.substring(url, url.length - 1);
        return pokemon;
    });
    return {
...
export function fetchPokemons() {
    return dispatch => {
        ...
        return fetch(`https://pokeapi.co/api/v2/pokemon/?limit=784`).then(response => response.json()).then(json=> {
